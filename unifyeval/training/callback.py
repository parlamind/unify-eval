...
from unifyeval.model.mixins.classification import Classifier, MessageLevelClassifier
from unifyeval.model.mixins.embedding import EmbeddingModel
from unifyeval.model.types import Label
from unifyeval.training.model_saver import QueuedModelSaver
from unifyeval.utils.corpus import group
from unifyeval.utils.data_collection import DataCollector
from unifyeval.utils.load_data import KeyedBatchLoader, KeyedSubsampledBatchLoader
...
class PlotParameters(TrainerCallback):
    """
    ...
    def __init__(self, folder_path: str):
        ...
        """
        self.folder_path = folder_path
        self._writer = SummaryWriter(logdir=self.folder_path)

    def __call__(self, model: DeepModel, i_minibatch: int, iteration: int, *args, **kwargs) -> DeepModel:
        for name, parameter in model.get_numpy_parameters().items():
            self._writer.add_histogram(tag=name, values=parameter, global_step=self.global_step)
        return model
