...
from unifyeval.model.deep_model import load_model
...
from unifyeval.training.callback import PlottingCallback, CheckNaN, EvaluationCallBack
...
from unifyeval.utils.load_data import FiniteKeyedLazyDataLoader, KeyedBatchLoader, KeyedSubsampledBatchLoader


class LanguageModelEvaluation(PlottingCallback):
...
class GenerateText(PlottingCallback):
...
class TextClassifierEvaluation(PlottingCallback):
    def __init__(self, folder_path,
                 corpus: Corpus,
        ...
        trainer = Trainer(data_loader=KeyedBatchLoader(raw_texts=self._X_train, labels=self._Y_train),
                          ...
                          batch_callbacks=[
                              ...
                              EvaluationCallBack.default(
                                  folder_path=os.path.join(self.folder_path, "train", f"{self.global_step}"),
                              ...
                              EvaluationCallBack.default(
                                  folder_path=os.path.join(self.folder_path, "test", f"{self.global_step}"),
                              ...
                              EvaluationCallBack.default(
                                  folder_path=os.path.join(self.folder_path, "test_85", f"{self.global_step}"),
                                  data_loader=KeyedBatchLoader(raw_texts=self._X_test,
                                                               labels=self._Y_test),
                                  label_indices=self.corpus.label_mapper.all_indices,
                                  junk_threshold=0.85,
