...
class AverageEmbeddingModel(PytorchSaliencyModel, EmbeddingModel, Classifier):
    ...
    def get_saliency_matrix(self, texts: ListOfRawTexts, label: int, max_length: int = None, **kwargs) -> Tensor:
        ...
        gradients = list(self.get_gradients(tensor=loss, with_respect_to=embeddings))
        self.average_embedding_classifier.zero_grad()
        saliency = -t.einsum("abc,acd -> ad", gradients, list(embeddings.permute((0, 2, 1)))).detach().numpy()
        return saliency
    ...
    def map_to_embedding_sequence(self,
        ...
        indices = t.from_numpy(self.sequence_mapper.encode_texts(tokenized_texts=tokenized_texts,
